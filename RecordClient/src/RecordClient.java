...
//		response.close();
		...
		//---------------------------Number in endpoint specifies which record to delete(recordId)
//		System.out.println("Deleting record from database");
//		response = client.target("http://localhost:8080/RecordManagement/webservice/records/25").request().buildDelete().invoke();
//		response.close();
//		response = client.target("http://localhost:8080/RecordManagement/webservice/collectors/Nahid").request().buildGet().invoke();
//		Collector putCollector = response.readEntity(Collector.class);
//		System.out.println(putCollector.toString());
//		response.close();
	...
	/*
	 * Utility class for creating and inserting example data
	 */
	...
	/*
	 * Utility class for creating example data
	 */
